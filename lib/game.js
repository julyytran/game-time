...
Game.prototype.savePoints = function(){
  ...
  // scoreboardRecords.push({[username]: points});
...
Game.prototype.loadScoreboard = function(){
  ...
  // $('.scoreboard-records').remove();

  fireDb.ref('highscore/').once('value').then(function(scores){
    var allScores = scores.val().highscores.sort(function(a, b){return b.points - a.points});
    ...
    for (var i = 0; i < scoreboardRecords.length; i++) {
      $('#scoreboard-records').append(allScores[i].username + ": " + allScores[i].points + "<br>");
    }
  });
};
...
//   fireDb.ref('highscore/').once('value').then(function(scores){
//     debugger
//     var allScores = scores.val().highscores.sort(function(a, b){return b - a});
//
//
//     for (var i = 0; i < 5; i++) {
//       if (allScores[i] !== undefined) {
...
//
//     for (var i = 0; i < scoreboardRecords.length; i++) {
//       $('.scoreboard-records').append("<li>" + scoreboardRecords[i] + "</li>");
//     }
//   });
// };

  ...
  for (var i = 0; i < sushis.length; i++){ //collision detection is something game should do
    ...
    if (nyanCat.x < currentSushi.x + currentSushi.width &&
     ...
     nyanCat.height + nyanCat.y > currentSushi.y) {
       clearSushi(currentSushi, sushis, i)
    }
  }
...
function clearSushi(currentSushi, sushis, index){ //sushis.splice(index, 1)
  sushis.splice(index, 1)
  // context.clearRect(0, 0, canvas.width, canvas.height); //call to function that clears sushi, clear from array?
  // nyanCat.draw();
  context.font="20px Georgia";
  context.fillText("You got points!", 320, 450);
}
